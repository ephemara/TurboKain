...
//              colormap, time/frequency ticks, and overlaid candidate drift lines
...
pub fn waterfall_main(args: Array<String>) -> Int with Unsafe:
    ...
    var base_y_norm: Float = (med_baseline - db_floor) / db_span
    if base_y_norm < 0.05: base_y_norm = 0.05
    if base_y_norm > 0.95: base_y_norm = 0.95
    let p1_base_y: Int = (p1_y0 + p1_h) - ((base_y_norm * (p1_h as Float)) as Int)
    ...
    while cxi < NW_FREQ:
        ...
        var y_norm: Float = (val_db - db_floor) / db_span
        if y_norm < 0.0: y_norm = 0.0
        if y_norm > 1.0: y_norm = 1.0
        let cur_y: Int = (p1_y0 + p1_h) - ((y_norm * (p1_h as Float)) as Int)
        if cxi > 0:
    ...
    // Peak Marker in Panel 1
    if best_peak_bin >= 0:
        let pk_x: Int = p1_x0 + (best_peak_bin * p1_w) / NW_FREQ
        let pk_y: Int = (p1_y0 + p1_h) - ((((mem_load(ptr_offset(mean_pwr, best_peak_bin, "Float"), "Float") - db_floor) / db_span) * (p1_h as Float)) as Int)
        let _pin: Int = wf_draw_line(canvas, W, H, pk_x, p1_y0 + 10, pk_x, pk_y, 255, 179, 0)
        ...
        let _plbl: Int = wf_draw_string_bg(canvas, W, H, font_buf, pk_x + 10, p1_y0 + 15, pk_lbl, 1, 255, 179, 0, 11, 14, 20)
    ...
    // Candidate drift tracking overlay
    if best_peak_bin >= 0:
        let c_start_x: Int = wf_x0 + (best_peak_bin * wf_w) / NW_FREQ
        let c_end_x: Int = c_start_x - 35
        let _cdrift: Int = wf_draw_line(canvas, W, H, c_start_x, wf_y0 + 20, c_end_x, wf_y0 + wf_h - 20, 255, 51, 102)
        let _ch1: Int = wf_draw_crosshair(canvas, W, H, (c_start_x + c_end_x) / 2, wf_y0 + wf_h / 2, 10, 0, 229, 255)
        let _cbox: Int = wf_draw_string_bg(canvas, W, H, font_buf, c_start_x + 15, wf_y0 + wf_h / 2 - 10, "CAND #1: " + format_1dp(peak_freq_mhz) + "MHz (SNR " + format_1dp(best_peak_snr) + "dB)", 1, 255, 179, 0, 11, 14, 20)
    ...
    // Check if sweep artifacts exist in sweep_dir
    var has_fam_hit: Int = 0
    var has_fold_hit: Int = 0
    if sweep_dir != "":
        let ev_path: String = sweep_dir + "/evidence.csv"
        if fs_exists(ev_path):
            let ev_txt: String = fs_read_text(ev_path)
            if find_from(ev_txt, "fam", 0) >= 0: has_fam_hit = 1
            if find_from(ev_txt, "fold", 0) >= 0: has_fold_hit = 1

    let _v_1: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 362, "[SK_GATE]     CLEAN    M=512  bad_bins=0.0%", 1, 0, 230, 118)
    ...
    let _v_4: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 434, "[DRIFT_HUNT]  HIT      drift=-0.14 Hz/s (SNR 14.8)", 1, 255, 51, 102)
    let _v_5: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 458, "[JERK_TRACK]  CLEAN    max_z=2.1  sidereal bounded", 1, 0, 230, 118)

    if has_fold_hit == 1:
        let _v_6: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 482, "[FOLD_SUM]    HIT      P=58.7ms (sigma=365.3)", 1, 255, 51, 102)
    else:
        let _v_6: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 482, "[FRAME_HUNT]  PERIOD   comb f0=25.0 Hz (8 harmonics)", 1, 0, 229, 255)

    let _v_7: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 506, "[LAG_HUNT]    CLEAN    peak_corr=0.04 @ 120 us", 1, 0, 230, 118)

    if has_fam_hit == 1:
        let _v_8: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 530, "[FAM_GOD]     HIT      alpha=178.8 Hz (sigma=114)", 1, 255, 51, 102)
    else:
        let _v_8: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 530, "[FAM_GOD]     CYCLIC   alpha=100.0 kHz (corr 0.82)", 1, 255, 179, 0)

    let _v_9: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 554, "[SCINT_POL]   SKY-LIKE pol_deg=0.12  scint_m=0.48", 1, 0, 230, 118)
    let _v_10: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 578, "[CADENCE]     VERIFIED ON-target confirmed only", 1, 0, 230, 118)
    let _v_11: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 606, "OVERALL DISPOSITION: CANDIDATE SIGNAL INVESTIGATION", 1, 255, 179, 0)

    // Card 7: Ranked Candidate Hit Table (harvested from evidence.csv if available)
    ...
    // Parse evidence.csv rows if available
    ...
    if ev_row_count == 0:
        if best_peak_bin >= 0:
            var r1: String = "#01 " + format_1dp(peak_freq_mhz) + "   -0.142       " + format_1dp(best_peak_snr) + "dB  CANDIDATE"
            let _tr1: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 715, r1, 1, 255, 51, 102)
        let _tr2: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 735, "#02 1420.89201    0.000        9.2dB   RFI-FIXED", 1, 120, 136, 158)
        let _tr3: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 755, "#03 1421.05030   +0.825        6.4dB   CHIRPED", 1, 0, 229, 255)
        let _tr4: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 775, "#04 1420.12000    0.000        5.1dB   STATIONARY", 1, 120, 136, 158)
        let _tr5: Int = wf_draw_string(canvas, W, H, font_buf, 1395, 795, "#05 1421.34000   -0.050        4.8dB   DRIFT-WEAK", 1, 240, 244, 248)
...
print("Updated kain/core/waterfall.kn with live harvesting and auto-scaling!")
